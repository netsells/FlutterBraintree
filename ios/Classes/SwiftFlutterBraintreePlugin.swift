...
public class SwiftFlutterBraintreePlugin: NSObject, FlutterPlugin {
    ...
    private lazy var btPaymentViewController: BraintreePaymentFlowViewController = {
        return BraintreePaymentFlowViewController()
    }()
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        if call.method == "start" {
            ...
            let nonce =  string(for: "nonce", in: call)

            let braintreeApi = BTAPIClient(authorization: authToken ?? "")
            let flowRequest = BTPaymentFlowDriver(apiClient: braintreeApi!)
            flowRequest.viewControllerPresentingDelegate = self.btPaymentViewController

            let threeDSecureRequest = BTThreeDSecureRequest()
            threeDSecureRequest.amount = NSDecimalNumber(value: amount ?? 0.1)
            ...
            flowRequest.startPaymentFlow(threeDSecureRequest) { (threeDSecureresult, error) in
                guard let secureResult = threeDSecureresult as? BTThreeDSecureResult, let tokenizedCard = secureResult.tokenizedCard else {
                    result(FlutterError(code: "braintree_error", message: "error occured when creating payment flow \(error?.localizedDescription)", details: nil))
                    return
                }
                if tokenizedCard.threeDSecureInfo.liabilityShiftPossible {
                        if tokenizedCard.threeDSecureInfo.liabilityShifted {
                            print("*** success from paymentflow")
                            result(tokenizedCard.nonce)
                        } else {
                            result(FlutterError(code: "braintree_error", message: "*** failed during payment flow", details: nil))
                        }
                    } else {
                        result(FlutterError(code: "braintree_error", message: "*** was not possible to authenticate", details: nil))
                    }
                }
            }
...
extension SwiftFlutterBraintreePlugin: BTThreeDSecureRequestDelegate  {
